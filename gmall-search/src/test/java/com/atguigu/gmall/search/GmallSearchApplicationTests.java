...
class GmallSearchApplicationTests {
    ...
    void importData(){
        ...
        do {
            ...
            Resp<List<SpuInfoEntity>> spuResp = this.pmsFeign.querySpusByPage(queryCondition);
            List<SpuInfoEntity> spus = spuResp.getData();

            //遍历spu，查询sku
            spus.forEach(spuInfoEntity -> {
                Resp<List<SkuInfoEntity>> skuResp = this.pmsFeign.querySkusBySpuId(spuInfoEntity.getId());
                ...
                if(!CollectionUtils.isEmpty(skuInfoEntities)){
                    ...
                    List<Goods> goodsList = skuInfoEntities.stream().map(skuInfoEntity -> {
                        ...
                        //查询搜索属性及值
                        Resp<List<ProductAttrValueEntity>> attrValueResp = this.pmsFeign.querySearchAttrValueBySpuId(spuInfoEntity.getId());
                        ...
                        //查询品牌
                        Resp<BrandEntity> brandEntityResp = this.pmsFeign.queryBrandById(skuInfoEntity.getBrandId());
                        ...
                        //查询分类
                        Resp<CategoryEntity> categoryEntityResp = this.pmsFeign.queryCategoryById(skuInfoEntity.getCatalogId());
                        ...
                        //查询库存信息
                        Resp<List<WareSkuEntity>> listResp = this.wmsFeign.queryWareSkusBySkuId(skuInfoEntity.getSkuId());
